...
namespace SocketLib
{
	...
	template<typename protocol, typename defaulthandler>
	void ListeningManager<protocol, defaulthandler>::SetConnectionManager(ConnectionManager<protocol, defaulthandler>* p_manager)
	{
		m_manager = p_manager;
	}
			...
			for (size_t s = 0; s < m_sockets.size(); ++s)
			{
				if (m_set.HasActivity(m_sockets[s]))
				{
					...
					catch (Exception& e)
					{
						...
						{
							std::cout << "Error: operation would blocl!\n";
							throw e;
						}
					}
				}
